...
def text_response(text: str) -> Dict[str, Any]:
    """Public alias for :func:`_text_response` — wrap a plain string in the
    MCP content shape. Used by the read-only research-tool server so the
    wire-shaping lives in one place."""
...
def unwrap_text_response(response: Dict[str, Any]) -> str:
    ...
    the shape is unexpected. Shared by the in-process callers (the role
    research tools and the Scholar executor) so the unwrap logic lives in
    one place."""
...
def run_bm25_search(args: Dict[str, Any], root: Path) -> Dict[str, Any]:
    ...
    the roles' read-only ``bm25_search`` research tool."""
...
def run_embedding_search(args: Dict[str, Any], root: Path) -> Dict[str, Any]:
    ...
    the roles' read-only ``embedding_search`` research tool."""
