...
The Pydantic AI agent run is monkey-patched (we stub
``librarian_agent.run_librarian_agent`` to return a typed
...
The agent itself (typed output, ModelRetry on malformed proposals,
read-only tools) is exercised in ``test_librarian_agent.py``.
"""
...
from agent_mem_daemon import librarian, librarian_agent, repair_queue
...
def _stub_agent(monkeypatch, *, proposal: LibrarianProposal, cost: float = 0.0, captured=None):
    ...
    monkeypatch.setattr(librarian_agent, "run_librarian_agent", _run)
...
def test_scan_agent_exception_swallowed(home, monkeypatch):
    ...
    monkeypatch.setattr(librarian_agent, "run_librarian_agent", boom)
...
def test_scan_agent_timeout_swallowed(home, monkeypatch):
    ...
    monkeypatch.setattr(librarian_agent, "run_librarian_agent", slow)
