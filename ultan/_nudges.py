...
# Project-bucket alias lookup, MIRRORED from ``tools/search/aliases.py`` rather
# than imported. The thin ``ultan`` install (without the ``[retrieval]`` extra)
# does not ship the agent-mem-search package, so ``from aliases import …`` would
# break the base install and its CI job (`uv sync --locked` with no extra →
# ModuleNotFoundError). ``_priming.py`` and ``_session_context.py`` mirror their
# cross-package logic for exactly this reason. These two functions are tiny and
# pure-stdlib; keep them byte-faithful to the canonical aliases module.
